...
**阶段 A（下一步）：同窗停靠**
主窗口布局支持停靠区：打开模块时主窗口按停靠位伸展（如 720×178 → 720×H），
模块面板填入停靠区；每模块可独立开关；主控退出即全部退出（本来就是单窗口单进程）。
不改窗口管理（亚克力/置顶/WndProc 全复用）。骨架 1~2 天。
...
1. **音频管线挂钩 + 均衡器**（2~4 天）：目前解码器直连 `player.append()`；需引入
   `Source` 链式管线构建器，EQ = biquad 链逐采样处理，可视化顺手拷采样。**这是解锁
   音频类模块的唯一硬重构，优先级最高。**
